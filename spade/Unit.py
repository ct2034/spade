...
class Unit(Behaviour.OneShotBehaviour):
    ...
    def createRoom(self):
        ...
        b = self.CreateRoomBehaviour(ID, self.muc_name, self.name, self.nick, self.parent, self.contentLanguage, self.type, self.goalList, self.password, self.agentList)
        ...
        #setting room configuration
            ...
            if msg and msg.getAttr("type") == "result":  # comprobar mejor el mensaje que se devuelve
                ...
                for agent in self.agentList:
                    ...
                    item = Node(tag="item", attrs={"affiliation": "member", "jid": agent})
    ...
    class AddOwnerBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            iq = Iq(to=self.name + "@" + self.muc_name, typ='set', attrs={"id": self.ID})
            query = Protocol('query', xmlns="http://jabber.org/protocol/muc#admin")
            item = Node(tag="item", attrs={"affiliation": "owner", "jid": self.newOwnerJID})
            query.addChild(node=item)
            iq.addChild(node=query)
            self.myAgent.jabber.send(iq)
            msg = self._receive(True, 10)
                ...
                if query:
                    ...
                    for item in query:
                        if str(item.getAttr("jid")) == self.OwnerJID:
                            i = Node(tag="item", attrs={"affiliation": "member", "jid": self.OwnerJID})
